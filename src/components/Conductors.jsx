...
import { getadmins,deleteadmin ,getdrivers,deletedriver,getconductors,deleteconductor} from '../redux/actions/admin'
...
function Conductors({data:companyOwnerInfo}) {

    const images=[imageOne,imageTwo,imageThree]
const [errorMessage, setErrorMessage] = useState("")
    const conductors=useSelector((state)=>state.getConductors);
    const {loading,error,data}=conductors;
  ...
  return (
    <MainLayout>
         ...
         <p id='single-trip-nav' style={{
            ...
            <p>
            <Link to="/">Home </Link>{'>'}  <span className='path-color'> Conductors </span> 
            </p>

<Link to="/add-conductor" style={{
    marginRight:'1rem'
}}>Add Conductor</Link>
</p>

<h3 className='h3-heading'>
              Find and add your Conductors here
            </h3>
         </header>
