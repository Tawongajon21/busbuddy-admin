...
function AddAdmin({data:companyOwnerInfo}) {
    let adminRegister=useSelector((state)=>state.adminRegister);
    console.log(adminRegister);
    const {getCompaniesloading,getCompaniesData,getCompaniesError} =CompaniesData();

  console.log(CompaniesData());
    const {loading,error,data}=adminRegister;
...
const signup=(e)=>{
    ...
    if (password!==confirmPassword) {
        ...
        dispatch(adminregister(email,phone,password,name,surname,company,signature))
    }
       ...
       window.location.replace(`${baseUrlFrontend}/admins`)
  ...
  console.log(companies);
    ...
    <MainLayout>
         ...
         <p id='single-trip-nav' style={{
            ...
            <p>
            <Link to="/">Home </Link>{'>'}  <Link to="/">Admins </Link>{'>'}  <span className='path-color'> Add Admin </span> 
            </p>
