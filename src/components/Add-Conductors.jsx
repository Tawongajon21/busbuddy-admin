...
import {adminregister,driverregister,reseterror,conductorregister} from "../redux/actions/admin"
...
function AddConductor({data:companyOwnerInfo}) {
    let conductorRegister=useSelector((state)=>state.conductorRegister);
    console.log(conductorRegister);
    const {getCompaniesloading,getCompaniesData,getCompaniesError} =CompaniesData();

  
    const {loading,error,data}=conductorRegister;
    console.log(conductorRegister);
...
const signup=(e)=>{
    ...
    if (password!==confirmPassword) {
        ...
        dispatch(conductorregister(email,phone,password,name,surname,company,signature))
    }
       ...
       window.location.replace(`${baseUrlFrontend}/conductors`)
  ...
  console.log(error);
    ...
    <MainLayout>
         ...
         <p id='single-trip-nav' style={{
            ...
            <p>
            <Link to="/">Home </Link>{'>'}  <Link to="/conductors">Conductors </Link>{'>'}  <span className='path-color'> Add Conductor </span> 
            </p>
