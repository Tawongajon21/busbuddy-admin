...
import { getadmins,deleteadmin ,getdrivers,deletedriver} from '../redux/actions/admin'
...
function Drivers({data:companyOwnerInfo}) {

    const images=[imageOne,imageTwo,imageThree]
const [errorMessage, setErrorMessage] = useState("")
    const drivers=useSelector((state)=>state.getDrivers);
    const {loading,error,data}=drivers;
  ...
  return (
    <MainLayout>
         ...
         <p id='single-trip-nav' style={{
            ...
            <p>
            <Link to="/">Home </Link>{'>'}  <span className='path-color'> Drivers </span> 
            </p>

<Link to="/add-driver" style={{
    marginRight:'1rem'
}}>Add Driver</Link>
</p>

<h3 className='h3-heading'>
              Find and add your Drivers here
            </h3>
         </header>
